...
def prepare(df, col_district, col_name, col_height, col_cca, col_year, col_lat, col_lon):
    # Drop rows without coords and coerce
    ...
    # Categories for faster filtering
    df[col_district] = df[col_district].astype("category")
    df[col_name]     = df[col_name].astype("category")

    # Precompute org-wide rankings
    org_ranks = {}
...
try:
    ...
    st.error("Dependency missing. Please ensure **openpyxl** is installed (it's listed in requirements.txt).")
    st.stop()
...
st.caption("Fixed-path, optimized app.")

# Filters (no path/sheet controls)
